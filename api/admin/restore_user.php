...
try {
    ...
    if (!isset($data['deleted_record_id'])) {
        throw new Exception("Missing data: deleted_record_id", 400);
    }

    // 3. Xử lý khôi phục người dùng
    $controller = new AdminController($conn, $current_user);
    $result = $controller->restoreUser($conn, (int)$data['deleted_record_id']);
